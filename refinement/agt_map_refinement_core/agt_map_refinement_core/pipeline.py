...
from .pcd import read_ascii_pcd, write_ascii_pcd
...
def refine_map_package(source_path: str | Path, refinement_path: str | Path,
                       output_path: str | Path, resolution: float = 0.05) -> Path:
    ...
    pcd = read_ascii_pcd(source / 'map.pcd')
    destination.parent.mkdir(parents=True, exist_ok=True)
    staging = Path(tempfile.mkdtemp(prefix='.refined-', dir=str(destination.parent)))
    try:
        for name in ('poses.txt', 'poses_timed.txt', 'metadata.yaml', 'patches', 'calibration.yaml'):
            source_item = source / name
            if source_item.exists():
                shutil.copytree(source_item, staging / name) if source_item.is_dir() else shutil.copy2(source_item, staging / name)
        ...
        write_ascii_pcd(staging / 'map.pcd', type(pcd)(pcd.header, pcd.fields, filtered_rows))
        shutil.copy2(rules_path, staging / 'refinement.yaml')
        write_nav_map(type(pcd)(pcd.header, pcd.fields, filtered_rows), rules.operations,
                      staging / 'nav_map.pgm', staging / 'nav_map.yaml', resolution=resolution)
